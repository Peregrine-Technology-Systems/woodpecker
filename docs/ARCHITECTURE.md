...
**Defense-in-depth telemetry (#268).** Because the submit-time rewrite is the *only* thing keeping deploy-class tasks off spot, a bypass (manual queue insert, a pre-rewrite restart race, a regression in the rewrite, or `WOODPECKER_TIER_DEPLOY_PATTERNS` unset) would silently route a release pipeline to a preemptible agent. To make that observable, the dispatch mirror in `server/rpc.Next` (`emitTierBypassIfSpotDeploy`) emits a `pipeline.tier_bypass` bus telemetry CloudEvent whenever a `spot`-tier agent actually claims a deploy-class task — which, with #266 working, never happens (a `tier=ondemand` task can't match a spot agent). It is **observe-only**: the task still runs (the agent has no decline RPC, so refusing would fail the release — worse than one spot run), and the signal is a *separate* code path from the rewrite, so it catches a rewrite regression that a server-side check colocated with the rewrite would miss. Detection is server-side, not agent-side, because spot agents are ephemeral (nothing to scrape) and can't reach the bus without a new RPC. Keyed on `tier=spot` exactly so the persistent `backend: local-d3ci42` box (untiered, runs deploy-class work legitimately) never trips it. Alert on a non-zero `pipeline.tier_bypass` rate. Re-routing a bypassed task to ondemand (a new agent-side decline RPC) is deferred to a follow-up.
